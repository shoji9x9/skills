...
- **本文を伴う外向き操作は、本文を先にファイルへ書き、`--body-file` / `-F` で渡す**。失敗時は再実行前に部分的な作成を確認し、存在すれば新規作成でなく補完する。
...
- `.agents/rules/eval-assertion-discrimination.md`: 回帰 eval のアサーション・fixture は書いた時点で「弁別・到達・材料・入力が答えを持っていないか」の 4 点を検証する。`skills/*/evals/**` 編集時に適用
