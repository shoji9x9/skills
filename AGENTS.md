...
major 更新は Dependabot（`dependabot.yml`）・`outdated.yml` のいずれも意図的除外で自動シグナルが出ない。
ランタイム非推奨（例: actions の node20 警告）のように major bump でしか解消できない問題は手動での定期確認が必要。
...
- `pr-finalize-loop`: 作成済み PR の CI エラー解消とレビュー指摘対応を、CI 成功かつ未解決スレッドなしになるまで自律ループで反復する。CI 失敗の修正・レビュー返信/解決・commit/push・再レビュー依頼を回す。
  依頼先は `skills.common.review_tool`（Copilot / Claude Code / Codex / none、既定 copilot）。`--max-iterations`（既定 5）・行き詰まり検知で停止。レビュー対応単体は `pr-review-handle` が担う
