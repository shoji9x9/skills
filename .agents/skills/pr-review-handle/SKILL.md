...
| `--push` | 上記に加え、関連ファイルだけを stage し、論理単位で commit、push する。push が発生した場合の完了条件（後述）を満たしてから締める |

`--push` は commit・push の実行をユーザーが明示的に委譲した合図として扱う。指定がない限り commit しない。
ただし、`--push` なしで一度止めた後にユーザーが commit / push を指示した場合も、**push が発生した時点で同じ完了条件を適用する**。
...
**push が発生した場合、この確認は作業の完了条件である。** `--push` で push した場合だけでなく、
`--push` なしでレビュー対応後にユーザーの追加指示で push した場合も同じ。Copilot 再レビュー依頼の方針
（今すぐ / CI 完了後 / 依頼しない）を確認し、選択に応じた処理を終えるまで final response してはいけない。
