...
- **作成・追記は非破壊**（`references/conventions.md` の設定ファイルの扱いと同じ要領）: ファイルが無ければ
  `.config/skills/shoji9x9/` ごと作成し、`skills.common.review_tool` だけを書く。既にあれば欠けたキーだけを
  `common` セクション（無ければ親も）に追記し、既存のキー・値・コメントは変更しない。値が既にあれば尊重し上書きしない。
