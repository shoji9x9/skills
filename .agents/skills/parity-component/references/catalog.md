...
- **現行に在る状態でも、基準を採っていない見本は置かない**（例: 採取の状態に無い「ポップアップを開いた状態」の見本）。
  照合の母集合に入らない見本は、部品の変更で状態を作れなくなっても（ポップアップが開かない等）どの照合・完了判定も落ちず、利用者がカタログを開くまで誰も気付かない。
  上の対応表の突き合わせで**カタログ側にしか無い見本**が見つかったら、その件数を `build-metadata.json` の `parity.unbaselined_stories` に記録し、1 以上なら完了しない。
  現行にその状態が在るなら採取の漏れなので、状態を足して `capture` へ戻り基準を採る。現行に無いなら次項の仕様変更として扱う
