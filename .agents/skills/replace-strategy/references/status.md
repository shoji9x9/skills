...
11. **要求単位が未確認の API**: features.md の機能一覧と横断 API 表について、**行ではなく口の単位で**未検証領域を導出する
    ...
    **導出は 2 つの集合の差分で行う**——行ごとに「API 列に並ぶ口の集合」と「根拠列のエントリが対応づく口の集合」を作り、次のどちらかに当たる口をすべて列挙する（slug と口を書く）。
    ...
    - **根拠の語彙が `実測:` でも `推定:` でもない口**（`実測できず: …` のような書き方。前方一致で `実測` に倒さず未確認として数える。
      機械検査（[`evidence.md`](evidence.md)「漏れを数える」）も同じ 3 条件で数えるので、ここを落とすと人手の導出とツールで判定が割れる）
...
13. **部品の洗い出しの未決**: `.replace/dependencies.md` の一覧から、**`状態` が `有効` で `決定` が `未確認` / `機能固有` / `内蔵` の行**を着手前の確認事項として列挙する
    （値の意味と引き取り手の正本は [`dependency-selection.md`](dependency-selection.md)「洗い出しの 6 値」）。
    - `未確認` は**未検証領域として数える**（確かめられていない種類は、その部品が要る機能の実装まで誰も見ない）。「理由・引き取り手」列の「確かめるのに要る条件」を添える
    - `機能固有` は「適用範囲」列の機能の `parity-replace` が実装フェーズ前に決める予定として示す（未決だが引き取り手が決まっている）
    - `内蔵` は「採用したもの」列のパッケージが同じ台帳で `パッケージ採用` の `有効` な行として決まっているかを突き合わせ、**決定が無い・取り消されている内蔵**を要確認として挙げる（誰も実装しない部品が残るため）
    - **同じ `部品（用途）` で `有効` の行が 2 つある**状態も要確認として挙げる（どちらが現在の決定かを台帳から決められない）
    - **行は在るのに `有効` が 1 つも無い部品**も要確認として挙げる（覆りの追記を飛ばした形。`状態` は唯一その場の更新を許される列なので
      追記専用の検査を通り、読む側には「行が無い」のと区別が付かない。「まだ見ていない」ではなく「現在の決定が失われている」として報告する）

    **ファイルが無ければ「部品の洗い出しが未実施（`setup` 手順 10 未実施）」と報告する**——無いことを「部品が無い」と読まない。
