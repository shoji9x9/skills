...
1. **機能ごとの現況表**: slug ごとに、Issue 状態（未起票／open／closed）、パリティスイートの有無と強度（`strength.md` の弱点・未検証種別を含む）、ベースラインの有無、
   ...
   `phase_b.<slug>.<target>.dataset_version` < 現在の `version` なら「その環境の新側データが陳腐化・要再投入」。投入対象でない target〈`dataset_mode: db` で `db` 未定義／`seedable` 無し〉は対象にしない〈契約の正本は [`project-config.md`](project-config.md)〉）、
