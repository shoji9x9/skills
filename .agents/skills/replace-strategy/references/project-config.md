...
- 要素は `id`（一意）・`regex`（呼び出しの字面に当たる JavaScript の正規表現。1 行ずつ照合する）・`kind`（種類）・`example`（`regex` が一致すべき実際の呼び出しの字面。一致しないパターンは走査 0 件を「呼び出しが無い」と区別できないため照合で落ちる）。**スキルは固定の名前を持たない**——名前はプラットフォームで決まるため、ここで宣言する
