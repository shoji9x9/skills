...
skills:
  replace-strategy:
    ...
    verification_commands: # 検証コマンド列（静的解析・単体テスト・統合テスト等。固有のツール名は設定側に置く）。対象プロジェクト側にコードを生成する全スキルが読む（下記「検証コマンド」）
...
静的解析・型検査・テストのコマンド列。**固有のツール名はスキル本体に書かず設定側に置く**ため、リポジトリのリント・フォーマッタ・テストランナーがそのまま効く。

- **環境準備・起動・URL 解決は含めない**（それらは `targets` の `pre_commands` / `start` / `check_urls`）。環境に依存しないコード検証なので、どの target でも同じ列を一律に実行する
- **読むのは対象プロジェクト側にコードを生成する全スキル。** ただし**未設定のときの挙動は用途で分かれる**:

  | スキル | 通す対象 | 未設定のとき |
  |---|---|---|
  | `parity-replace` | 新側の実装（＋付随する IaC 差分） | **停止する**（完了判定が「新で green ＋検証コマンド」なので成立しなくなる） |
  | `golden-dataset` | 投入ツール | 停止せず、`verification.md` に記録して進む |
  | `parity-suite` | スイート・マッピング層・操作アダプタ | 停止せず、`gaps.md` に記録して進む |
