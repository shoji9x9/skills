...
skills:
  replace-strategy:
    ...
    intentional_diffs: # 意図的差異レジストリ
      ...
      #   ブロックに収める（読み手が続きの行を連結するのは名指しした鍵の値だけ。途中の空行・コメント行は
      #   またぐが、閉じないまま兄弟のキー・リスト要素へ出る・文書が終わる形は読めない）。読めないと行のまま
      #   突き合わせるので、**要素を足しただけの編集が縮小に化ける**。
      #   そう指摘されたときも **[] を復元せず、表記を 1 行へ直す**。
      #   **比較元（commit 済みの版）に閉じていないコンテナがあると、どう直しても通らない**——
      #   検査はその旨を出すので、表記を直したうえで内容を人が確認して通す。
      #   折り返したときの**注記は鍵の行か閉じる行に置く**。keep / may_change / pending は要素行に付いた
      #   行末注記を単位にしない（移動先に置き場所が無く、ブロック形式でも単位にしていない）ので、
      #   鍵の注記を要素行へ移すと単位が 1 件減って落ちる。独立したコメント行はどちらでも単位に残る。
