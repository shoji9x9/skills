...
9. **機能インベントリ**: 現アプリを機能単位に分解し、各機能のページ・API・テーブル・副作用出力、横断 API の fan-out と参照テーブル、slug を `.replace/features.md` に記録する。
   ...
   **API の口を書く前に、入口ごとの問い合わせを読んで「母集合」と「1 行が表すもの」を書き出し、その根拠を「要求単位の根拠」列へ口ごとに `実測` / `推定` で記録する**——
   ...
   **行に 1 つだけ書かない**（1 行が複数の口を持ち根拠が割れることは普通にあり、1 つでも実測があると行全体が実測に見えて未確定の口が `status` から落ちる）。
   入口の問い合わせが読めないときは証拠の階梯を上げ、API / UI の実動作で確定したものも `実測` として記録してよい（観測していない部分は推測で補わない）。
