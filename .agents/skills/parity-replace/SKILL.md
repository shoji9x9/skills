...
3. **部品の洗い出しと依存の決定**: **入る前に骨格（`references.architecture`）の未整備を検出し、未整備なら停止する**（挙動は上記キー表。骨格を自分で決めない）。
   ...
   `setup` で決定済みの共通部品はここで再決定しない。**例外は `決定` が `機能固有` の行**で、これは `setup` が「1 機能でしか使わないので採否はここで決めない」と申し送った部品なので、
   `適用範囲` がこの機能の slug なら**ここで採否を決めて差し替えとして追記する**（申し送りを引き取らないと、どの工程も決めないまま実装が始まる）。**実装中に必要と分かった部品も、そのまま自前実装で進めず同じ基準で判断して `.replace/dependencies.md` へ追記する**（`porting.md` の該当実装単位にも一行残す）。
