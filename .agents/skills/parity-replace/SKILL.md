...
3. **部品の洗い出しと依存の決定**: **入る前に骨格（`references.architecture`）の未整備を検出し、未整備なら停止する**（挙動は上記キー表。骨格を自分で決めない）。
   ...
   `setup` で決定済みの共通部品はここで再決定しない。**例外は `決定` が `機能固有` と `未確認` の行**で、どちらも `setup` が採否を決めずに送った部品である
   （`機能固有` は「1 機能でしか使わないので決めない」、`未確認` は「状態を作れず現行に在るか確かめられなかった」）。
   `機能固有` は `適用範囲` がこの機能の slug なら、`未確認` はこの機能の実装でその部品が要ると分かったときに、**ここで採否を決めて差し替えとして追記する**
   （`未確認` は現行に在るかも未確定なので、まず現行で確かめてから決める。どちらも引き取らないと、どの工程も決めないまま実装が始まる）。**実装中に必要と分かった部品も、そのまま自前実装で進めず同じ基準で判断して `.replace/dependencies.md` へ追記する**（`porting.md` の該当実装単位にも一行残す）。
...
4. **実装（フェーズごと）**: 現行コードをフロント・バック**いずれもロジックの一次情報源として読む**。照合単位を振り分ける（バックエンド＝旧新を並べた diff、フロントエンド＝スイート green か `parity-diff` 差分ゼロ）。
   ...
   **判定はファイルの有無ではなく、その部品が実際に先に作られているか**で行う——`.replace/components.md` は「先に作らない部品」も併記するので、
   ...
   「先に作らない部品」表にある・一覧に無い・build 成果物が無い部品は、このフェーズで普通に実装する（改修規律は適用しない）。
   **例外は「内蔵として扱う部品」節にある部品**で、これは採用ライブラリが描くので**実装しない**（節へ置いた判定が覆っていれば部品一覧の行になっているので、そちらに引き当たる）。先に作られていた場合は——
