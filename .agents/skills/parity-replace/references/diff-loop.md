...
- **単体の完了を「現行と一致」と報告しない**——現行との比較結果を報告できるのは往復ループの終了（`diff-metadata.json` の `converged: true`）の後だけで、そのときも承認済みの例外・意図的差異・未検証領域を並べて報告する（収束は生の差分ゼロではない）。報告の書き方は `SKILL.md` 手順 8「完了の報告」
