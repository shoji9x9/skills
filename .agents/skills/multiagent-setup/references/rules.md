...
- 全ファイルに適用する場合は `**` を使用する（ただし `**` はどのファイルでも常に守るべき普遍則に限る。下記「rules 化とスコープの基準」参照）
- 複数パターンは、`applyTo` はカンマ区切りの 1 文字列（例 `"src/**/*.ts,src/**/*.tsx"`）、`paths` は配列で 1 行ずつ列挙する。いずれも種別のみの広域指定にせず対象ディレクトリまで絞る
