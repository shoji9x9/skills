...
**`capture_conditions.states` を「見た目が変わる状態を思いついた分」だけで決めない。** 挙げなかった状態は画素では撮られず、特性照合は名前を付けた要素しか見ず、aria は既定の状態しか採らない——
...
- **数えた器を撮る／撮らないの両方で `capture_conditions.popup_inventory` に 1 行ずつ残す**（形式はテンプレート `assets/metadata-template.json`）。
  撮るなら `captured` に `states` の状態名、撮らないなら `captured: null` と `reason` を書き、同じ理由を `gaps.md`「特性化できなかった箇所と理由」へ種別「撮影状態の対象外」として残す。
  **欄が無いと「数えたが撮らないと決めた」と「思いつかなかった」が同じ見え方になる**
- `opened_by` には開く操作を操作アダプタの関数名（または状態名）で書き、2 段目以降は親の器の `name` を `parent` に書く。
  **書き終えたら操作アダプタ（`suite.interactions`）で器を開く関数を列挙し、全てが `opened_by` に現れることを突き合わせる**——追加した操作が撮影状態に反映されない抜けはここで拾う
...
- **2 標本の一致は採取が決定論的である証明ではない。** 2 値のどちらかに転ぶ採取は 1/2 の確率で「ノイズ 0」になる。
  ...
  待ちが落ち着かず例外になった状態は基準値を記録せず、`capture_conditions.states` から外し（`popup_inventory` の該当行は `captured: null` と `reason` にする）、`gaps.md` に「撮影状態の対象外」として残す
