...
- **右クリックは発火を観測して送り方を決める。** `locator.click({ button: 'right' })` で発火しない実装では、操作用ロケータで引いた可視要素の `boundingBox()` を取得する。
  矩形が `null` でなく幅・高さがともに正で、中心座標の `document.elementFromPoint()` が操作用要素または子孫を返す場合だけ、
  ...
  [`mouse.click`](https://playwright.dev/docs/api/class-mouse#mouse-click)・[HTML `elementFromPoint`](https://html.spec.whatwg.org/multipage/nav-history-apis.html#dom-document-elementfrompoint)）。
