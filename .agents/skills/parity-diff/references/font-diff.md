...
- **名前を付けた要素の差なら、採取物の `child_inline_styles`（`parity-suite` の trait-capture.mjs が採る 1 段下の子の inline style）を読む。**
  ...
  **比べるのは文字列ではなく宣言の集合**——`style` 属性は宣言の順序・空白・大小文字で簡単に食い違うので、
  プロパティ名と値の組に正規化してから突き合わせる。そのうえで**描画に効く差**（`font-weight` / `color` / 余白等の差、または子の在否）があるときだけ
  「フォント差ではなく装飾の乗り場所の差」として切り分けへ進まず `parity-replace` へ差し戻す。
  正規化後に一致する、または差が描画に出ない項目（`cursor` / `user-select` 等）だけなら**この分岐へ入らず**、下の切り分けの手順へ進む
