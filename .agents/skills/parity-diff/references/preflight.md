...
| 前提スキルの未解決の保留 | `.replace/dataset/pending-decisions.json` と `.replace/parity/<slug>/pending-decisions.json` に `resolution: null` の要素が無い（ファイルが無いのは保留なし。正本: `replace-strategy` の `references/autonomy.md`「下流の前提判定」） | 保留を抱えた `golden-dataset` / `parity-suite` |
