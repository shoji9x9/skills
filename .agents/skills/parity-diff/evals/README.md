...
- eval 5〜11・13・15 は前提の有無に関わらず会話で判定できる契約を対象にする（eval 15 は同一原因の N インスタンスに対する承認の粒度と、台帳の件数を畳まない規則の両立）。eval 14 は fixture で共同居住機能の新側 root が存在しない状態を作り、現側由来の矩形による target ごとの再導出・両画像への適用・解除へ到達することを検証する
