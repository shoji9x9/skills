...
- eval 6〜10・13 は前提の有無に関わらず会話で判定できる契約（他機能待ちの `blocked_by` 帰属と再判定トリガーの所在／レジストリごとに効く経路／「許容」確定の 2 段階／仮説検証の観測条件／新側採取スペックの雛形と `testIgnore` 除外／承認済み例外の根拠の宛先とインスタンス件数の可視性）を対象にする
