...
1. **状態取得**（待機の間隔・上限は後述「ポーリングと待機」に従う）
   ...
   - HEAD にまだレビューが付いていなければ Copilot にレビューを依頼し、付くのを上限つきで待つ（後述「ポーリングと待機」「Copilot 再レビュー依頼」）
2. **収束判定**: CI が全成功 **かつ** 未解決スレッドが無い → **完了**。サマリーを報告して終了する（レビュー出現待ちがタイムアウトしレビュー未着のまま閉じた場合は、その旨も明記する）
...
新しい HEAD にレビューが付いていなければ依頼する。push が発生した反復では、push・CI 再実行のあとに依頼する（壊れた HEAD でレビューを促さない）。
...
- 依頼後、`requested_reviewers` に `Copilot` が登録されたことを確認する（`gh pr view <番号> --repo <owner>/<repo> --json reviewRequests --jq '.reviewRequests[].login'` に `Copilot` が現れるか）。登録されていなければ依頼は成立していないので、その旨を報告する。
