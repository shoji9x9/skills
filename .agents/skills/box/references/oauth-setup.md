...
3. 取得した `code` を 30 秒以内に交換し refresh token を保存する（`<skill>` はインストール先のスキルディレクトリ）。認可コードは `ps` / `/proc` への露出を避けるため引数では渡さず、プロンプトに貼り付けるか stdin で渡す。
   ...
   自動実行では stdin でも渡せる。ただし `ps` / `/proc` 露出を確実に避けるには、認可コードを argv に載せない形で流し込むこと。`printf` がシェル組み込みなら（bash / zsh / dash など、このスキルが前提とする bash 系では組み込み）値は argv に載らないが、`printf` が外部コマンドの環境では argv に見え得る。露出を確実に避けたいときは上の対話入力を使う。

   ```bash
   # 例（printf が組み込みのシェルで実行する前提）
   printf '%s' '<code>' | bash <skill>/scripts/box-oauth-init.sh
   # 露出を確実に避けるなら、コードをファイルに置いて stdin に流す
