...
> フックの実行 cwd は通常プロジェクトルート。プロジェクト内（`.agents/` `.claude/` `.github/` 配下）ならそのまま相対パスでスクリプトを解決でき、ユーザースコープ（`~/.claude/...` 等）にある場合は絶対パスを使う。
> なお各スクリプト内の `.kaizen/` 参照はプロジェクトルート基準（`$CLAUDE_PROJECT_DIR`、未設定なら git ルート）で解決するため、フックがサブディレクトリ cwd で起動しても影響を受けない。
