...
2. **対象 transcript を同定し、持ち主かどうかで読み方を分ける。** 抽出しているセッション自身が持ち主なら、
   出来事は既に自分の文脈にあるので、transcript を読み直すぶんは二重の入力になる。しかも transcript は
   **文脈窓を超える大きさに育つ**（数 MB 規模＝数 M トークン）ため、指示どおりに読むことはそもそもできない。
   持ち主かどうかは**モードではなく transcript ごと**に決まる（`--all` は複数の transcript を回るので、そのうち自分のぶんだけが持ち主側の扱いになる）。

   **持ち主の判定**: ゲートにブロックされた場合は、ゲートが示した transcript パス・session id が自分のもの。
   直接 `kaizen --current` を指示された場合は、**いま追記されている最新のセッションログ**を起点に置く
   ...
   **センチネルの一致 0 件は「持ち主でない」の根拠にならない**——`setup` 未実行で Stop フックが無い、既に解消済み、といった理由でも 0 件になる。
   ...
   持ち主でない側（読む）へ倒すのは、対象が明らかに他セッションの transcript のとき（`--all` の他セッション分、センチネルの引き受け）と、
   一致が複数あって自分のものを決められないときだけにする。**0 件を理由に文脈窓を超える transcript を読み始めない。**

   **持ち主のセッション**: **一次情報は自分の文脈**にし、transcript は**候補の位置特定**にだけ使う。ゲート経由かどうかに関わらず走査器を使う:

   ```bash
   bash <スキル>/scripts/kaizen-candidate-scan.sh "<transcript>" ".kaizen/.extract-checkpoint.<session key>"
   ```

   出るのはカテゴリ（`user correction` / `tool error` / `repeated edit`）と**絶対行番号**だけで、transcript の本文は出ない。
   これを索引として自分の文脈の該当箇所を辿り、**思い出せない行があるときだけその周辺だけ**を読む（圧縮が入った後は要約しか残っていないため）。
   **transcript 全体を読まない。**
   行番号が印字されるのは**先頭 5 件まで**で、総数は `N candidate(s) found` の側に出る。印字された行を候補の全件と読まない
   （総数が 5 を超えるなら、印字されていない候補が自分の文脈側にある前提で最重要 1 件を選ぶ）。
   走査器の exit 1（検証済みゼロ）は「候補パターンに当たらなかった」であって「学びが無い」ではないので、自分の文脈に残る失敗をこれで打ち消さない
   （走査器の判定を候補ゼロの根拠にしてよいのは、限定契約でそう定めた `--record-pending` だけ）。
   exit 2（判定不能）は「候補なし」に倒さない。持ち主のセッションでは**索引が得られなかっただけ**なので、自分の文脈から候補を挙げて先へ進む
   （ここで transcript 全体を読み直す側へは倒さない。`--record-pending` だけは契約どおり BLOCKED にする）。

   **持ち主でない transcript**（`--all` のうち他セッションのぶん、他セッションのセンチネルを引き受けた場合）:
   出来事が文脈に無いので従来どおり読み込む（`--current` なら前セッションの最新ファイル 1 つ）。

   どちらの場合も、その transcript の処理位置 `.kaizen/.extract-checkpoint.<session key>`（1 行目 transcript パス / 2 行目 バイト位置 / 4 行目 行数）が
   その transcript を指しているなら、**その位置より後だけ**を対象にする。**初回抽出でも checkpoint はありうる**——
   同じセッションの前の commit がゲートを候補ゼロで自動通過していれば、ゲートが `kaizen-extract-done.sh --checkpoint-only` で
   checkpoint だけを進めている。「まだ 1 度も抽出していないから offset は無い」と決め打ちせず、**ファイルの実在を確かめる**。
   session 単位の名前になる前の旧 checkpoint `.kaizen/.extract-checkpoint` しか無く、その 1 行目が同じ transcript を指す場合は、
   ゲートと同じくそれを走査器へ渡す。checkpoint を無視して全体を対象にすると、既に記録済みの学びを重複して起票してしまう
   （同一セッションで 2 回目以降の抽出は、複数 commit する branch では普通に起きる）
...
7. 承認された候補を `.kaizen/YYYY-MM-DD-<slug>.md` に書き込む（`status: pending`）。frontmatter の直後に学びの要約を表す H1 見出し（`# <学びの要約タイトル>`）を 1 行付ける（markdownlint MD041 を満たし、読み物として自然にするため）
   ...
   **書き込んだら、そのリポジトリの整形の入口を通す。** 抽出はコミット前ゲートから駆動されるので、
   利用者は記録が書かれたらすぐ同じ `git commit` を再実行する。その commit の pre-commit 検査には
   **いま書いたばかりの記録が入る**——書式検査を持つリポジトリでは、記録そのものが次の commit を落とす
   （実測: 表の記法で 8 件。ステージしていなくても、検査が `git ls-files --cached --others` で集める構成では対象になる）。
   記録は追跡対象に置く前提なので、`.gitignore` へ入れて検査から外す逃げ道は使えない。
   **整形のコマンドはリポジトリごとに違うので固定のコマンドを書かない。** そのリポジトリの整形の入口
   （`package.json` の `format` 系スクリプト、`Makefile` の `fmt` ターゲット、pre-commit の整形ステップ等）を
...
8. 抽出が完了したら、バンドルされた `kaizen-extract-done.sh` でセンチネル削除と処理位置 `.kaizen/.extract-checkpoint.<session key>` の記録を行う
   ...
   **削除の空振りは stderr の警告で分かる**——`rm -f` は対象が無くても正常終了するため、終了コードでは「解消した」と「解消するものが無かった」が同じ値になる。
   `kaizen-extract-done: 警告: 削除対象のセンチネルがありませんでした` が出たら、消えたつもりで先へ進まず、`--session-id` / `--sentinel-suffix` が**センチネルを立てた本人**のものかを確かめ直す。
   スクリプトはリポジトリの全作業ツリー（本体＋git worktree）の `.kaizen/` を対象にするので、共有ツリーと worktree にまたがったセッションでも解消できる（Issue #344）。
...
>
> **先頭段落を折り返さない**: 供給されるのは最初の非空行**だけ**なので、先頭段落を複数の物理行に
> 折り返すと、注入される要約は文の途中で切れた断片になる（強調記法も閉じないまま入る）。注入は
> 成功し終了コードも 0 のため失敗としては現れない。`kaizen-status-check.sh` が pending の学びを対象に
> この形を検出して exit 2 にする（先頭行が箇条書き項目で、次行も箇条書き項目なら兄弟項目とみなし
> 折り返しとは扱わない）。長い先頭段落は折り返さず 1 行に収めるか、1 文へ削る。
