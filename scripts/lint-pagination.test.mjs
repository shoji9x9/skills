...
test("tokenize: クォート内の空白を 1 トークンに保つ", () => {
  expect(tokenize('gh api -H "Accept: application/vnd.github+json" repos/o/r/issues')).toEqual(
    ["gh", "api", "-H", '"Accept: application/vnd.github+json"', "repos/o/r/issues"],
  );
});
...
test("GraphQL: first: に pageInfo も --paginate も無ければ指摘", () => {
  expect(count("t.md", md("gh api graphql -f query='{ a { reviewThreads(first: 50) { nodes { id } } } }'"))).toBe(1);
});

test("GraphQL: pageInfo があれば許可", () => {
  expect(count("t.md", md("gh api graphql -f query='{ reviewThreads(first: 50) { pageInfo { hasNextPage } nodes { id } } }'"))).toBe(0);
});
...
test("GraphQL: # pagination-ok で抑制", () => {
  expect(count("t.md", md("gh api graphql -f query='{ reviewThreads(first: 50) { nodes { id } } }'  # pagination-ok"))).toBe(0);
});
...
test("REST: 変更系（POST 等）は対象外", () => {
  expect(count("t.md", md("gh api --method POST repos/o/r/pulls/6/requested_reviewers -f reviewers[]=Copilot"))).toBe(0);
});
