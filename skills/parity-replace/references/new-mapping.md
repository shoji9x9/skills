...
新側の分岐を足しても、撮影状態へ遷移する関数が撮る対象の矩形が落ち着くまで待ってから返す約束は崩さない（分岐から早期 return して待ちを迂回すると、新側採取でだけ 1 画素の揺れが 2 値に転ぶ。正本は `parity-suite` の `references/baseline.md`「撮る対象が動かなくなるまで待つ」）。
