...
- **本スキル単体の完了**: 選択した target に対しパリティスイートが**新で green** ＋ 検証コマンド（設定 `verification_commands.full`。**完了判定は常に全体走査の列**）が通る。**`parity-diff` の差分ゼロは含めない**——含めると `parity-diff`（前提に本スキルの完了を要求する）と循環する
- **往復ループ全体の終了条件**: `parity-diff` が**未説明差分ゼロ かつ 未修正回帰ゼロ**（全差分が系統差／宣言済み例外に分類済み）。「許容」例外の確定には**ユーザー承認**が要る（`parity-diff` 側で扱う）
- **単体の完了を「現行と一致」と報告しない**——現行との比較が収束したと報告できるのは往復ループの終了（`diff-metadata.json` の `converged: true`）の後だけで、そのときも承認済みの例外・意図的差異・未検証領域を並べて報告する（収束は生の差分ゼロではない）。報告の書き方は `SKILL.md` 手順 8「完了の報告」
