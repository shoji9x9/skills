...
10. **共通部品の依存決定**: 複数機能で使う部品（UI ライブラリ・フォント・状態管理・日付処理等の基盤と、ボタン・セレクト・モーダルダイアログ等の個々の UI プリミティブの両方）を洗い出し、**自前で書くか／どのパッケージを使うか**を実装が始まる前に決めて `.replace/dependencies.md` に記録する。
   ...
   **確かめた種類はどれも台帳の行として残す**。採否に至らない結果にも値がある（`該当なし` / `内蔵` / `機能固有` / `未確認`）ので、
   行を作らない扱いを設けない（値の意味・書く工程と読む工程・覆ったときの記録方法の正本は同ファイルの「洗い出しの 6 値」。初期表示に出ない種類の扱いは「洗い出しの網羅」節）。
   **確かめられなかった種類を `該当なし` に倒さない**（`未確認` の行にし、理由と確かめるのに要る条件を書く）。
...
- **追記専用（非破壊追記）の成果物は機械可読な一覧を正本にする**——[`assets/append-only-manifest.json`](assets/append-only-manifest.json)。
  ...
  **パスはルートからの完全なパス**——設定ファイルなら `skills.<スキル名>.intentional_diffs.pending` であって末尾だけでは引かず、一致しないパスは黙って「外し忘れ」になる）・
  ...
  **全部を行として比べない**——正本が明示的に求めるその場の更新（版の +1・状態列の `未`→`済`・Issue 列の `未起票`→番号・最終更新の日時）が
  ...
  `lines` ではさらに**フロー形式のコンテナが育った**ことによる行の書き換え（`keep: []` → `keep: ["…"]`）を縮小に数えない——
  空リストへ最初の要素を足す書き手は必ずこの形を通り、棚卸しで `pending` から `keep` へ移した文言も `keep` の行の書き換えになる。
  判定は**鍵ごとの多重集合**で行い、その鍵のコンテナ行の件数が減らず、要素が全部現在側にも在るときだけ緩める。
  行はインデントを畳むので同じ鍵の行が複数ありうる（`targets[].forbidden_actions` 等）——失われた行ごとに独立へ判定すると、
  **中身の違う兄弟が同じ 1 件の育ったコンテナを重複して根拠にできる**。
  **行末コメントは切ってから読む**（正本の設定ファイルは `keep: [] # 変えない（…）` と書く。行末が `]` であることを求めると実プロジェクトで緩和が一度も効かない）。
  コメント自体は**鍵の一部**として扱い、消した行は別の鍵になって緩和から外れる。
