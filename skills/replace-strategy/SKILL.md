...
- **追記専用（非破壊追記）の成果物は機械可読な一覧を正本にする**——[`assets/append-only-manifest.json`](assets/append-only-manifest.json)。
  ...
  **鍵だけを同一性にしない**——表の行は先頭セルだけ残せば中身を丸ごと差し替えられ、`key` つきの配列も鍵以外が自由に書き換わる。
  ...
  （どちらも**パスはルートからの完全なパス**——設定ファイルなら `skills.<スキル名>.intentional_diffs.pending` であって末尾だけでは引かず、一致しないパスは黙って「外し忘れ」になる）
  ...
  **全部を行として比べない**——正本が明示的に求めるその場の更新（版の +1・状態列の `未`→`済`・Issue 列の `未起票`→番号・最終更新の日時）が
  ...
  `lines` で「空リストへ最初の要素を足す」「棚卸しで `pending` から `keep` へ文言を移す」を縮小に化けさせないのが `growable_containers` である。
