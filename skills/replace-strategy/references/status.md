...
行の被覆に加えて**部分**〈機能行はページ ＋ 新規実装 API の口 ＋ 比較対象の副作用出力、横断 API 行は口 ＋ 参照テーブル、バッチ行は入力 ＋ 比較する出力〉の集合差分も取り、覆われない部分を挙げる）
    ...
    **features.md の「受け入れ条件」列は読まない**——列は突き合わせの出力（写し）であり、本文が後から書き換われば黙って古くなる（「状態」列と同じ理由）。
    合わせて**同じ Issue 番号が 2 つ以上の行（機能・横断 API・バッチ）の Issue 列に入っている状態**を要確認として挙げる——
