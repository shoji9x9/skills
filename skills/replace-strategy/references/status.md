...
| 情報源 | 読むもの |
|---|---|
| `.replace/features.md` | 機能・横断 API・バッチ・その他の Issue（4 種以外）の一覧、slug、fan-out、ページ一覧（ページ × 乗る機能）、ページ要素の帰属（要素 × 配置の所有者 slug）、**API の「要求単位の根拠」列（`実測` / `推定`）**、Issue 番号（**番号だけ。旧版テンプレート由来の「状態」列があっても読まない**——下記「Issue 状態の取得」） |
...
11. **要求単位が未確認の API**: features.md の機能一覧と横断 API 表から、**「要求単位の根拠」に `推定` のエントリを 1 つでも含む行**を未検証領域として列挙する。
    **行ではなく口の単位で示す**——根拠は口ごとに書かれ（正本は [`features-issues.md`](features-issues.md)「API の形は要求単位を読んでから決める」）、
    1 行に `実測` と `推定` が混在する（一覧の `GET` は読めたが更新・削除の要求単位は未確定、等）。
    **行に 1 つでも `実測` があることを根拠に行ごと除外しない**——除外すると未確定の口が報告から落ち、この列を作った目的が失われる。列挙には slug と**該当する口**を書く。
    採番は特性化より先に来るためこの列は未実測の画面についても埋まっており、`推定` の口は**実装のほぼ最後まで「決まっている形」として読まれる**（着手時に確定すべき口が残っている、というシグナル）。
    **列そのものが無い features.md では「要求単位の根拠が未導出（テンプレート更新前の features.md）」と報告する**——列の不在を「全て実測済み」と読まない。
    **空欄のセル、および口に対応づかないエントリだけのセルは `推定` と同じく未確認として数える**（`実測` に倒さない）
