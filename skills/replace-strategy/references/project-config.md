  ...
  replace-strategy:
    current: # 現行アプリ（URL・DB・認証・禁止操作は targets の side: current が持つ）
      ...
      feedback_calls: # 移行元の利用者に見える副作用を出す呼び出しの一覧（下記「フィードバック呼び出し」）。キー欠落＝未確認（parity-suite が確認して 1 回記録する）
        - id: toast # パターン id（一意）
          regex: '\bshowMessage\s*\(' # 呼び出しの字面に当たる JavaScript の正規表現（ファイル全体に複数行モードで照合。^ / $ は行頭・行末）
          kind: toast # 利用者に見える副作用の種類（toast / script / dialog 等）
          example: "showMessage('Saved', 'success')" # regex が一致すべき実際の呼び出しの字面（陽性コントロール）
    ...
    intentional_diffs: # 意図的差異レジストリ
      ...
      #   そのため setup が同じ実行で保留を書き出す場合は、[] を書いてから差し替えるのではなく最初からブロック形式で書く
      #   （上の「キーを必ず空リストとして作る」は保留が 0 件のときの形。0 件でないなら初期値を経由しない）。検査側の恒久対応は Issue #426。
...
- 要素は `id`（一意）・`regex`（呼び出しの字面に当たる JavaScript の正規表現。ファイル全体に複数行モードで照合し、^ / $ は行頭・行末に効く）・`kind`（種類）・`example`（`regex` が一致すべき実際の呼び出しの字面。一致しないパターンは走査 0 件を「呼び出しが無い」と区別できないため照合で落ちる）。**スキルは固定の名前を持たない**——名前はプラットフォームで決まるため、ここで宣言する
...
- **`diff` を回す工程でも、変更集合が次のいずれかを含むなら `full` へ前倒しする**（差分限定では原理的に捕まらないため、レビュー往復の前に安価に落とす）:
  ...
  - 共有された定義に対する、**変更集合の外の利用側が満たさなくなる追加**——型・スキーマ・インターフェースへの**必須プロパティ／必須要素の追加**、
    網羅的に扱う共用体・列挙への**分岐の追加**など（その型のリテラルを書いている別ディレクトリ・別パッケージが、変更集合に入らないままプロパティ不足で型検査に落ちる）
- **前倒しの条件を操作名（削除・改名・追加）で覚えない。** 判断は機構で行う——**その変更が、変更集合の外にあるファイルの判定を変えるか**。
  変えるなら前倒しする（差分限定で穴が開くのは**判定が他ファイルに依存する検査**〈型検査・未使用エクスポート検出・参照解決〉であり、操作名はその機構の例示にすぎない）。
  **利用側のテストが緑であることを前倒し不要の根拠にしない**——スタブに足りないプロパティが `undefined` になり、
  応答の形を `toStrictEqual` で固定した assertion が古い形を通すなど、テストは型の不足を見ないことがある
...
- **`retention` は版の話であり、同一版の 2 回目の採取（ノイズ測定用）には及ばない。** ノイズ基準値・新側の自己ノイズを測るための 2 回目の採取物は、
  基準値を記録したら削除する**一時作業物**として扱い、`storage` の選択にもコミット対象（テキストは Git）にも入れない
  （手順の正本は `parity-suite` の `references/baseline.md`、新側は `parity-diff` の `references/capture-new.md`）。
...
| `added_by` | 必須 | 追記したスキル名（`replace-strategy` / `golden-dataset` / `parity-suite` / `parity-replace` / `parity-component`）。旧形式からの移行で復元できないものだけ `unknown`。**この一覧に無い名を書くのも、書き手が分かっているのに `unknown` と書くのも帰属不明へ倒れる**ので、新しい書き手を増やすときはこの行と `pending-triage-check.mjs` の集合を同時に更新する |
...
- **帰属を信用できるのは `added_by` が読めているときだけ。** `added_by` が無い・`unknown`・未知のスキル名の要素は、
  ...
  どの機能の棚卸しにも出続けるので、早めに `keep` / `may_change` へ移すか書き手を復元する
...
- **`replace-strategy` の追記も必ず `slug: cross-cutting` にする。** 意図的差異レジストリを作るのは `setup` の手順 8 で、**機能 slug の採番は手順 9**なので、追記の時点で書ける機能 slug がまだ無い
  （順序の根拠は `SKILL.md` の `setup` の手順）。採番後に機能へ帰属させられると分かった保留は、**人が** `keep` / `may_change` へ移すか、その機能を担当するスキル（`golden-dataset` / `parity-suite` / `parity-replace`）が機能 slug で書き直す
