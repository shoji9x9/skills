...
<!-- （該当が無いと確かめた種類は .replace/dependencies.md に「該当なし」で残す）。 -->
<!-- .replace/dependencies.md で 内蔵 / 機能固有 / 未確認 / 該当なし の行（状態が「有効」のもの）になった種類はこの部品一覧に載せない——誰も先には作らないため。 -->
<!-- 機能固有は下の「先に作らない部品」表にも載せない（あちらはインスタンスが 1 件しか無い共通部品の置き場で、こちらは機能に閉じた部品）。 -->
<!-- その値が後から覆って共通部品になったら、dependencies.md 側で差し替え追記したうえでこの一覧へ行を足す。 -->
