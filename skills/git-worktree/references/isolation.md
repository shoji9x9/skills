...
「コマンドの形」の検査は広い。**プログラム名・引数を実行時に組み立てる呼び出しは、git を含まなくても拒否される**
（実測で 8 回踏んだ形）。

- 変数で組み立てる引数（`S=<dir>; node $S/x.mjs`）
- 複合コマンド（heredoc でファイルを書いて `wc -l` まで繋げる等）・`for` ループ
- コマンド置換を含む待機（`until [ "$(gh api ...)" != "0" ]; do sleep 30; done`）
- ヒアドキュメント（**追記 `>>` でも同じ**。scratchpad だけに書く場合も拒否される）

**組み立てが必要な処理はスクリプトファイルへ書いてから絶対パスで実行する。書く手段までがこの手順に含まれる**——
スクリプトはヒアドキュメントでは作れない（それ自体が拒否される）。ファイル書き込みツール（Claude Code なら
`Write` / `Edit`）で作り、`chmod +x` と実行を**別々の呼び出し**に分ける。

**判定は字面で決まる。** 本文・コメント・置換文字列にバージョン管理コマンドの名前が現れるだけで拒否されるので、
「触る対象が scratchpad だから通るはず」という推論は当たらない。

**待機・ポーリングを要する手順**（`pr-finalize-loop` のレビュー待ち等）はとくにこの形に当たりやすいので、
最初からスクリプトファイルへ書く前提で設計する。

**記録が注入されていても、コマンドを書く瞬間には参照されない**（この制約は pending の学びとして注入され続けた
まま 5 回再発した）。`priority: high` の学びは注入で凌がず apply を先に通す。
