...
`multiagent-setup` スキルの動作をテストケースで検証する手順。
...
```bash
mkdir -p tests/multiagent-setup/iteration-N
```
...
結果は tests/multiagent-setup/iteration-N/ に保存すること。
```
...
python -m scripts.aggregate_benchmark \
  /path/to/tests/multiagent-setup/iteration-N \
  --skill-name multiagent-setup \
  --skill-path /path/to/skills/multiagent-setup
```
...
cat tests/multiagent-setup/iteration-N/benchmark.md

# 前回との差分
diff tests/multiagent-setup/iteration-<前回>/benchmark.md \
     tests/multiagent-setup/iteration-N/benchmark.md
```
