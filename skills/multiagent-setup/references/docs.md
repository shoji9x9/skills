...
- 対象読者に馴染みがない可能性のある用語は、必要な場合に使い、初出時に平易な言葉で短く説明する
...
この節は用語を禁止したり、すべての説明を長くしたりするためのものではない。応答方針は上記の短い節だけにまとめ、用語集・文章検査 Hook・エージェント固有ファイルへの複製は追加しない。
...
同じタイミングで、基底ドキュメントに上記 `## コミュニケーション` の方針が無ければ追加する。既に同等の方針がある場合は重複させず、既存の構成と表現を尊重する。
