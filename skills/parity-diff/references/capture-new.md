...
- **現側 `capture_conditions.popup_inventory` を読む**（正本: `parity-suite` の `references/baseline.md`「撮影状態の決め方（2）器の棚卸し」）。
  ...
  **`opened_by` が空配列の行・同じ要素を 2 つ持つ行・同じ `parent` × 同じ呼び出しを持つ 2 行・同じ `name` の 2 行（`parent` を問わない）も不整合として停止する**（前 2 つはどの器がその呼び出しで開くかが決まらず、最後は 1 つの器が 2 回数えられている——開き方が複数あるなら行を分けず 1 行の `opened_by` に並べる）。
