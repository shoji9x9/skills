...
- 雛形は 1 回目（`baseline-new/`）と 2 回目（`noise-pass2/`）を同じスペックの別パスとして撮る。差分量（`pixel_diff` / `trait_diffs`）を測るのは記録済みの差分器の仕事で、スペックは撮るだけ。
  **2 回目は測定後に削除する一時作業物**（下記「2 回目の採取物は測定後に削除する」）
...
| `popup_inventory` | 現側の `popup_inventory` が整合したか（`captured` が全て `states` の要素・`captured` を持つ行は `reason: null`・`captured: null` の行は空でない `reason`・器を開く呼び出し〈親の器 × 関数名 × 開く対象の論理名〉が全て同じ `parent` の行の `opened_by` に現れる・`opened_by` が空でなく行内に重複が無く同じ `parent` × 同じ呼び出し／同じ `parent` × 同じ `name` が 2 行に現れない。単一の文字列の `opened_by` は旧形式として 1 要素の配列と同義に読む）と、`diff.md` の未検証領域へ転記した `captured: null` の器。キーごと無い旧成果物は停止し、ユーザー承認の例外で続行したときだけ `"absent: 承認済みの例外。ノイズ吸収なしで続行"`（不整合は停止） |
...
- **現側 `capture_conditions.popup_inventory` を読む**（正本: `parity-suite` の `references/baseline.md`「撮影状態の決め方（2）器の棚卸し」）。
  ...
  **操作アダプタ（`suite.interactions`。新側例外を含む）とスイートから器を開く呼び出しを「どの器の中から呼ぶか（1 段目は `null`）× 関数名 × 開く対象の論理名」の単位で列挙し、
  全てが同じ `parent` を持つ行の `opened_by`（`<関数名>(<開く対象の論理名>)` の配列）に現れることも確かめる**——
  関数名だけで突き合わせると、引数で対象を変える関数の 1 行が他の呼び出しまで満たしてしまい、文脈を落として突き合わせると、別の親の器から呼ぶ同じ呼び出しが 1 つに潰れる。
  現れない呼び出しがあれば、その器は数えられておらず撮られていないので停止し `parity-suite` へ戻す。
  **`opened_by` が空配列の行・同じ要素を 2 つ持つ行・同じ `parent` × 同じ呼び出しを持つ 2 行・同じ `parent` × 同じ `name` の 2 行も不整合として停止する**（前 2 つはどの器がその呼び出しで開くかが決まらず、最後は 1 つの器が 2 回数えられている——開き方が複数あるなら行を分けず 1 行の `opened_by` に並べる）。
  **単一の文字列の `opened_by` は旧形式として 1 要素の配列と同義に読む**（読み替えるのは形だけで、上の突き合わせと不整合の判定は同じに当てる）。
  列挙した呼び出しは `opened_by` と同じ正規化（**開く対象を決める引数だけを残し、それ以外は全て落とす**。正本は `parity-suite` の `references/baseline.md`）に揃えてから突き合わせる——
  表記が揃っていないと、同じ呼び出しが 2 つの文字列になり「現れない呼び出し」として偽の停止を作る。
  **記録側に旧い引数表記（`openCombo(page, 演算子)` 等）が残っている場合も同じ理由で偽の停止になる。** これは不整合ではなく移行漏れなので、
  停止せず記録側を新しい表記へ書き換えてから突き合わせる（正本は同じく `baseline.md`）。
...
- **`noise-pass2/` は保存先選択の対象外**（測定後に削除するため。下記「2 回目の採取物は測定後に削除する」）
...
**残すのは測定値だけで、2 回目（`new/<target>/noise-pass2/`）の採取物そのものは残さない。** ゲート判定も再利用判定も読むのは
`diff-metadata.json` の `noise_baseline_new` / `noise_measurement` であり、**記録後の採取物はどの工程も読まない**（現新比較の相手は 1 回目の `baseline-new/`）。

- **削除は測定手順の一部**。`diff-metadata.json` へ書いた直後に `noise-pass2/` を削除し、そこまで済ませて測定完了とする
- **コミットしない**（特性 JSON・aria はテキストだが、`baseline-new/` と同量の重複がリポジトリへ入るため区分を当てない）
- **不在は失効条件ではない。** 下記「失効条件」の判定材料は前回実行の記録値と `fingerprint` であって採取物ではないので、
  採取物が無いことを理由に再測定へ倒さない（逆に、採取物が残っていることを再利用の根拠にもしない）
- 中断で `noise-pass2/` が残っていた場合も入力ではないので破棄してよい。測り直す組は同じ場所へ撮り直す
- **現側も同じ扱い**（`parity-suite` が測るノイズ基準値の 2 回目。正本は `parity-suite` の `references/baseline.md`）
