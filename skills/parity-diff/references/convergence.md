...
- 収束の条件:
  ...
  - **部品被覆表に未測定が残っていない**（正本は `parity-suite` の `references/coverage.md`「部品被覆表」）。
    ...
    `kind: non-renderable` なのに一意な locator・状態の導出源・`states_exhaustive: true`・重複のない `expected_states` と `states[].name` の完全一致・
    1 件以上の状態別証拠（遷移、矩形、`offset_parent`、0 寸法または `display: none` / `visibility: hidden | collapse` の非表示原因）が揃わないもの、
