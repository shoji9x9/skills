...
- **ストレージ実体への投入は v1 スコープ外**（`storage.seedable: true` でも投入しない。正本は `replace-strategy` の `references/project-config.md`「ファイルストレージ」）。
  新側のストレージにゴールデンデータのファイルが無い前提で進め、**ストレージ実体に依存する検証は「未投入＝未検証」として `verification.md` のフェーズ B 節に明記する**（説明済みのデータ差として扱わない）
- **アップロード用の入力ファイルは写像の対象外**（同じバイト列を現・新の両側でそのまま使う）。ただし**保存後の path 命名規則が新側で変わる場合は意図的差異**であり、
  宣言が無ければ `intentional_diffs.pending` へ追記してユーザー確認へ回す（下記「新規の意図的差異の扱い」と同じ扱い）
- 新側の受け皿がファイル形式（`dataset_mode: static`）の場合の写像は従来どおり本ファイルの写像設計に従う（ストレージ実体の話とは別軸）
