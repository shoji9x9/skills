...
```text
/
├── skills/dependabot-merge/
│   └── evals/
│       ├── evals.json              ← テスト定義（プロンプト + アサーション）
│       └── README.md               ← この文書
└── tests/dependabot-merge/
    └── iteration-N/
...
```bash
mkdir -p tests/dependabot-merge/iteration-N
```
...
```text
skill-creator を使って skills/dependabot-merge のスキルを検証したい。
スキルの場所は skills/dependabot-merge/SKILL.md。
evals/evals.json のテストケースを使って回帰テストを実行したい。
結果は tests/dependabot-merge/iteration-N/ に保存すること。
```
...
cat tests/dependabot-merge/iteration-N/benchmark.md

# 前回との差分
diff tests/dependabot-merge/iteration-<前回>/benchmark.md \
     tests/dependabot-merge/iteration-N/benchmark.md
```
