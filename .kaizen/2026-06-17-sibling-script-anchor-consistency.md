...
適用状況（applied）: kaizen 系スクリプト 3 本すべてを `$CLAUDE_PROJECT_DIR`（未設定なら
`git rev-parse --show-toplevel`、git 外は cwd）の同一アンカーに統一した（Issue #34 / PR #36）。

- `kaizen-archive.sh`: ルートへ `cd`（失敗時はメッセージ付きで `exit 1`）。
- `kaizen-context-inject.sh` / `kaizen-precommit-gate.sh`: ベストエフォートで `cd`（失敗時は現状 cwd
  で続行＝従来挙動を保つ。フックはセッション/コミットを止めない原則を優先）。

各スクリプトは単体配布・個別インストールのため共通ソースの sourcing 機構を持たず、約 1 行のアンカー
ブロックを各スクリプトに同一コメント付きで複製している（共有ヘルパー化はインストール/バンドルを
複雑にするため見送り）。ルール文面の `.agents/rules/` への明文化は任意の補強で、必要になれば別途行う。
