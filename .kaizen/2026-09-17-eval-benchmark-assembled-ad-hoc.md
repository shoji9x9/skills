...
境界は `scripts/build-skill-eval-benchmark.test.js` で固定し、判定行を無効化する 15 変異が
狙ったテストを落とすことを `scripts/check-mutation-proof.js` で実証した（`build-skill-eval-benchmark.mutations.json`）。
