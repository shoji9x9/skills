...
同 PR の Codex レビュー（#463）でさらに 2 件: 並行に出た 2 つの移動を結果の到着順で確定していた（実行順は不明）、
`eval "$MOVE"` のように字面に `cd` が現れない移動を見逃していた。いずれも根拠の文法（順序・展開）を軸として表にしていなかった。
その後も `\cd` / `c'd'` / 行継続 / `c$'d'` / `c$(printf d)`・CDPATH と、拒否リスト（移動の綴りの列挙）を 4 巡拡張し続け、
最後に許可リスト（全コマンド語が素の字面のときだけ cwd を保つ）へ切り替えた。このノートの提案が既に許可リストを言っていたのに、注入されていても適用されなかった。
...
- 発動条件を明文化する: 同じ緩和に「別の綴り」の指摘が 2 巡続いたら、拒否リストに足さず許可リストへ書き換える（rule §3 の「同じクラスを 3 巡指摘されたら入力形式を疑う」と対にする）
