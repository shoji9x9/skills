...
eval 38 は、市販データグリッドの列見出しで「固定プロパティ集合が全一致で緑なのに画素だけ差が出る」状況から、
...
|---|---|---|---|
| 38 採取対象が描かれているか | **7/7** | 2/7 | 5（写しの矩形・論理名の付け直し・採取ツールの失敗と欠落へ変換しない規約・`child_inline_styles`・マッピング修正後の採り直し） |
...
`origin/main` 側で別内容の eval 37 が先に入ったため、この eval の id は 38 に採番し直した（採点内容は同じ）。
