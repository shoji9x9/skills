...
- eval 33 は撮影状態の器の棚卸し（Issue #360）の回帰。被覆表の present を根拠に 2 段目の器を撮影状態から外し、別機能で撮る器を何も書かずに外す案を押し戻せるかを見る。
  ...
  Issue #364 で 1 つの器を複数の操作で開ける場合（クリックとキーボード）と、同じ呼び出しを別の親の器から呼ぶ場合をプロンプトへ足し、assertion 6（行を分けず `opened_by` の配列に並べる）・7（`parent` × 呼び出しの単位で識別する）を追加した。
  プロンプトが置くのは「2 行に分けるつもり」「同じ呼び出しを 2 つの親から呼んでいる」という事実だけで、行の単位・配列・照合の単位は書かない。
  iteration-40 の内訳は**弁別が 3・4・5・6**（6 は `without_skill` が「2 行にするのは正しい」と逆の結論を出した）、**後退検知が 1・2・7・8**——
  7（`parent` × 呼び出しの単位）は「容器が違えば 2 行・2 枚」と自力で到達するため Delta には寄与しない
...
- eval 37 は部品とインスタンスの集合の来歴（Issue #392 / #393）の回帰。受領ソースが読めるのに画面を歩いて部品とページを洗い出し、
  ...
  いま渡したときの採り直しの範囲を対象にする。プロンプトにはキー名・ファイル名・ツール名を書かない。
  iteration-33 では `without_skill` も assertion 1（母集団が自己申告なので未測定 0 は完全性の証拠にならない）・4（観測値と定義値は別）・6（注記は測定の代わりにならない）・8（矛盾する結論の併記が無い）に到達した——
...
- eval 39 は採取物の健全性（Issue #382）の回帰。実体だけ採り直して展開結果が旧いまま、読み手のいない採取物が在る状態から、
  ...
  **prompt の「読み手の検査はどこまで見れば足りる？」は、あえて契約と反対の答えを示唆する形**にしてある——
...
- eval 40 は状態を変えるスイートの反復実行（Issue #382）の回帰。後始末を外のコマンドで流し、1 回の緑と強度ゲートの素通り 0 件で
  ...
  `suite.state_mutating` / `suite.repeat_run` への記録を対象にする。
