...
fixture 付き eval（`evals.json` に `fixture` があるもの）は `--fixture evals/parity-diff/<fixture の値>` を足して実行する（例: eval 4 は `--fixture evals/parity-diff/fixtures/green-only-local-dev`）。
