...
- `skills.common.review_tool`: pr-review-handle / pr-finalize-loop が再レビューを依頼する AI レビュアー。`copilot`（既定）/ `claude-code` / `codex` / `none`。
  ...
  一時的な個人設定は Git 管理外の `.env.local` に `SKILLS_REVIEW_TOOL=codex` のように書く。`mise.toml` がこのファイルを読み込み、環境変数は共有設定より優先される。1 回だけ変える場合は `pr-finalize-loop 6 --review-tool <tool>` のように対象 PR も指定する。
